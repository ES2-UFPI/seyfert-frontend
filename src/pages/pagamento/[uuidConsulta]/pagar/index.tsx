...
const PagamentoConsulta = ({ uuidConsulta }: PagamentoConsultaProps) => {
  ...
  const [opcaoPagamentoEscolhido, setOpcaoPagamento] = useState(
    opcoesPagamento[0]
  );
  ...
  return (
    <Layout title="Pagamento da consulta" titleHeader="Pagamento da consulta">
      <div className={styles.container}>
        <p>{uuidConsulta}</p>
        <button className={styles.btn_voltar} onClick={cancelarPagamento}>
          Voltar
        </button>
        <div>
          <div className={styles.area_pagamento}>
              ...
              {opcoesPagamento.map((elemento, index) => {
                return (
                  <ButtonComponent
                    ...
                    color={
                      compare(elemento, opcaoPagamentoEscolhido)
                        ? "rgb(29, 66, 230)"
                        : "rgb(177, 177, 177)"
                    }
            ...
            <center>
              <div className={styles.area_pagamento_body}>
                {!!opcaoPagamentoEscolhido &&
                  renderizadorComponent(opcaoPagamentoEscolhido)}
              </div>
            </center>
          </div>
        </div>
